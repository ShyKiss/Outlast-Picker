...
var Config Bool DisableClickSound, DisableTeleportSound, DisablePause, DisablePickerDebug, DisableTimerDebug, DisableShowStamina, DisableMenuMusic;
...
Exec Function ToggleClickButton() {DisableClickSound=!DisableClickSound;}
Exec Function TogglePauseMenu() {DisablePause=!DisablePause;}
Exec Function ToggleTeleportButton() {DisableTeleportSound=!DisableTeleportSound;}
Exec Function TogglePickerDebug() {DisablePickerDebug=!DisablePickerDebug;}
Exec Function ToggleTimerDebug() {DisableTimerDebug=!DisableTimerDebug;}
Exec Function ToggleShowDebugOnlyStamina() {DisableShowStamina=!DisableShowStamina;}
...
Function DrawHUD() {
    ...
    if(PickerPawn.GodMode==true) {HealthPlayer="God";} else if(PickerPawn.GodModeInNoclip==true) {HealthPlayer="Noclip";} else {HealthPlayer="" $ PickerHero(Controller.Pawn).Health;}
    ...
    PTimer(0.016);
    if(ToggleHUD) {PickerFunc(); return;}
}
...
Event PickerFunc() {
    ...
    JDB( Vect2D(640 - 250, 250), Vect2D(500, 250), MakeRGBA(20,20,20,180), StartClip, EndClip);
    Controller = PickerController(PlayerOwner);
    ...
    JDS(">" @ Command $ "_", vect2D(590, 375), MakeRGBA(170,170,170,255));
    ...
    Switch(CurrentMenu)
    {
        case Normal:
            ...
            AddButton("Close", "TogglePickerMenu 0", vect2d(825, 480), false);
            break;
        case Player:
            AddButton("GodMode: " $ PickerHero(Controller.Pawn).GodMode, "ToggleGodMode", vect2d(425, 275),, StartClip, EndClip);
            ...
            AddButton("Limp: " $ PickerHero(Controller.Pawn).bLimping, "Limp",, true);
            AddButton("Hobble: " $ PickerHero(Controller.Pawn).bHobbling $ "/" $ PickerHero(Controller.Pawn).HobblingIntensity, "Hobble ",, true,,, true);
            AddButton("Batteries: " $ Controller.NumBatteries, "Batt ",, true,,, true);
            AddButton("FOV: " $ PickerHero(Controller.Pawn).DefaultFOV $ "/" $ PickerHero(Controller.Pawn).RunningFOV $ "/" $ PickerHero(Controller.Pawn).CamcorderMaxFOV, "ChangeFOV ",, true,,, true);
            AddButton("Damage: " $ 100-PickerHero(Controller.Pawn).Health, "DS ",, true,,, true);
            AddButton("Camera Bone: " $ PickerHero(Controller.Pawn).Camera.CameraBoneName, "CameraBone ",, true,,, true);
            ...
            AddButton("Player Scale: " $ PickerHero(Controller.Pawn).Mesh.Scale, "HeroScale ",, true,,, true);
        ...
        case World:
            ...
            break;
        case Enemy:
            AddButton("Kill Enemy", "KillEnemy", vect2d(425, 275),, StartClip, EndClip);
        ...
        case AddL:
            ...
            AddButton("Remove All Lights", "RemoveAllPickerPointLights",, true);
...
Function IntPoint GetMousePos() {local PickerInput PlayerInput; PlayerInput = PickerInput(PlayerOwner.PlayerInput); return PlayerInput.MousePos;}
...
Function AddButton(String Name, String ConsoleCommand, optional Vector2D Location, optional Bool AutoDown=False, optional Vector2D Bound_Start, optional Vector2D Bound_End, optional bool Template, optional String buttontag) {
    ...
    //Draw the button box
    JDB(Location, offset, Color, Begin_PointCalc, End_PointCalc);
...
Function JDB(Vector2D Begin_Point, Vector2D End_Point, optional RGBA Color=MakeRGBA(255,255,255,255), optional out Vector2D Begin_Point_Calculated, optional out Vector2D End_Point_Calculated) {
...
Function Commit() {local PickerController Controller; Controller = PickerController(PlayerOwner); if(!Controller.InsanePlusState) {PlayerOwner.ConsoleCommand(Command);} Command="";}
Exec Function HideDebugMenu() {PlayerOwner.ConsoleCommand("TogglePickerMenu 0"); Super.HideMenu();}   
